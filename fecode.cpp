...
/*	if (((GenRandom < 2) || (MaxGenerate = 1)) && (strlen(OutputFileName) > 0) && (!forcea))
	...
	{*/
		...
		{
			if (a[n] <= 0)	//lowercase
				res[n] = (char)(floor((a[n] * 10) + 0.1) + 25) + 'a';
			else
				res[n] = (char)(floor((a[n] * 10) + 0.1) - 1) + 'A';
		};
//	};
	res[n] = '.';
	return res;
...
void writedefaultcolormap(FILE *f)
{
	...
	for(n = 0; n < 252; n++)
	{
		...
		else if (n < 126)
		{
			r = 0;
			g = 255;
			b = (int)(blendfact * (n-84));
		}
		else if (n < 168)
		{
			r = 0;
			g = (int)(255 - ( blendfact * (n-126) ));
			b = 255;
		}
		else if (n < 210)
		{
			r = (int)(blendfact * (n - 168));
			g = 0;
			b = 255;
		}
		else
